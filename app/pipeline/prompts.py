...
MICRO_SYSTEM = (
    ...
    "You score sector ETFs using a strict TWO-PASS process:\n"
    "  PASS 1 (Top-Down): Scan the macro key events and apply the mandatory "
    "sector score constraints listed in the Macro Event Transmission Rules. "
    "These constraints OVERRIDE individual ticker news for the affected sectors. "
    "A commodity supply shock or geopolitical conflict must dominate the score "
    "for directly impacted sectors — do not let noisy ticker-level news suppress it.\n"
    "  PASS 2 (Bottom-Up): Use portfolio holdings, ticker news, sector outlooks, "
    "and earnings flags to refine the remaining sectors and fine-tune the constrained ones "
    "within the allowed bounds.\n"
    "Your final scores must reflect BOTH passes."
)

MICRO_USER = (
    "Today is {date}.\n\n"
    "## Macro Context\n{macro_context}\n\n"
    "## PASS 1 — Macro Event Transmission Rules (apply BEFORE reading ticker news)\n"
    "Scan the key_events in Macro Context above. If any event matches a category below, "
    "the score constraints for that category are MANDATORY. They override ticker-level news.\n"
    "Multiple categories can apply simultaneously — combine all relevant constraints.\n\n"
    "| Event Category | Trigger Examples | Mandatory Score Constraints |\n"
    "|---|---|---|\n"
    "| Energy supply shock | Hormuz closure, OPEC cut, pipeline attack, oil embargo, Russia oil ban "
    "| XLE >= 7; XLB >= 6; XLI <= 5; XLY <= 4 |\n"
    "| Commodity price spike | Oil/gas surge, metals rally, broad input inflation "
    "| XLE >= 6; XLB >= 6; XLP <= 5 (margin squeeze) |\n"
    "| Geopolitical conflict / war risk | Military strike, invasion, sanctions, armed conflict escalation "
    "| XLI >= 7 (defense contractors); XLE >= 6; XLY <= 4; XLF <= 5 |\n"
    "| Broad risk-off / financial stress | Credit spreads widen, bank crisis, VIX spike, systemic risk "
    "| XLV >= 7; XLP >= 7; XLU >= 7; XLY <= 3; XLK <= 4; XLF <= 4 |\n"
    "| Fed hawkish / rate hike | Rate hike, hawkish Fed, tightening, yield surge "
    "| XLRE <= 4; XLU <= 4; XLF >= 6 |\n"
    "| Fed dovish / rate cut | Rate cut, dovish pivot, QE, easing "
    "| XLRE >= 6; XLU >= 6; XLF <= 5 |\n"
    "| Recession / growth slowdown | PMI contraction, GDP miss, mass layoffs, recession signals "
    "| XLV >= 7; XLP >= 7; XLU >= 6; XLY <= 3; XLI <= 4 |\n"
    "| AI / tech catalyst | AI breakthrough, semiconductor boom, mega-cap tech beat "
    "| XLK >= 7; XLC >= 6 |\n\n"
    "If NO key event matches any category, skip Pass 1 and proceed directly to Pass 2.\n\n"
